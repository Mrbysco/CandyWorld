...
public class CustomLakesFeature extends Feature<BlockStateFeatureConfig> {
	private static final BlockState AIR = Blocks.CAVE_AIR.defaultBlockState();

	public CustomLakesFeature(Codec<BlockStateFeatureConfig> configCodec) {
		super(configCodec);
	}

	public boolean place(ISeedReader reader, ChunkGenerator generator, Random random, BlockPos pos, BlockStateFeatureConfig stateFeatureConfig) {
					...
					for(int i3 = 0; i3 < 16; ++i3) {
						for(int i4 = 0; i4 < 8; ++i4) {
							if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
								reader.setBlock(pos.offset(l1, i4, i3), i4 >= 4 ? AIR : stateFeatureConfig.state, 2);
							}
						}
					}
				}
					...
					for(int j2 = 0; j2 < 16; ++j2) {
						for(int k3 = 0; k3 < 16; ++k3) {
							for(int k4 = 0; k4 < 8; ++k4) {
								...
								if (flag1 && (k4 < 4 || random.nextInt(2) != 0) && reader.getBlockState(pos.offset(j2, k4, k3)).getMaterial().isSolid()) {
									reader.setBlock(pos.offset(j2, k4, k3), ModBlocks.CRYSTALLIZED_SUGAR.get().defaultBlockState(), 2);
								}
							}
						}
					}
